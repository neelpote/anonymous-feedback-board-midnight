...
async function deploy() {
  ...
  const zkConfigPath = path.resolve('contracts', 'managed', 'feedback');
  const contractModule = await import(path.resolve(zkConfigPath, 'contract', 'index.js'));
  const compiledContract = CompiledContract.make('feedback', contractModule.Contract).pipe(
    CompiledContract.withWitnesses(feedbackWitnesses),
    CompiledContract.withCompiledFileAssets(zkConfigPath),
  );
  console.log('Feedback contract loaded.');
  ...
  const providers = {
    privateStateProvider: levelPrivateStateProvider({
      privateStateStoreName: 'feedback-private-state',
      accountId,
  ...
  // Constructor arguments: surveyId (Bytes<32>), adminPubkey (Bytes<32>)
  const surveyId = Buffer.alloc(32); // All zeros initially
  const bootstrapContract = new contractModule.Contract(feedbackWitnesses);
  const bootstrapState = bootstrapContract.initialState(createConstructorContext(initialPrivateState, '0'.repeat(64)), surveyId, new Uint8Array(32));
